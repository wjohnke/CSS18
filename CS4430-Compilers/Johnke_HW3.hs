...
uniquemain (Program []) = Right "No main procedure"
uniquemain (Program fds) = case (l) of
								0 -> Right "No main procedure"
								1 -> Left $ Program fds
								_ -> Right "Too many main procedures"
	where
		l = length (filter (=="main") (extractName fds) )

extractName :: [FunDecl] -> [Name]
extractName [] = [""]
extractName ((FunDecl t name _ _ cmd):fds) = name:(extractName fds)
--
...
symboltable (Program fds) = genTable fds 

genTable :: [FunDecl] -> SymbolTable
genTable [] = []
genTable ((FunDecl t f params locals cmd):rest)= (t,f,params,(params++locals)):(genTable rest)
...
noundefined (Program fds) = checkVars fds

checkVars :: [FunDecl] -> Bool
checkVars [] = True
checkVars (fun:rest) = if (subseteq (usedvars fun, defvars fun))
					       then checkVars rest
						   else False
--
...
defvars (FunDecl _ _ params locals _) = (unwrap params)++(unwrap locals)

unwrap :: [Decl] -> [Name]
unwrap [] = []
unwrap ((Decl name t):rest) = name:(unwrap rest)
...
varsCmd :: Cmd -> [Name]
varsCmd c = case c of
			  (Assign name e)      -> (name):(varsExp e)
			  (While e c1)         -> (varsExp e)++(varsCmd c1)
			  (Seq c1 c2)          -> (varsCmd c1)++(varsCmd c2)
			  (IfThen e c1)        -> (varsExp e)++(varsCmd c1)
			  (IfThenElse e c1 c2) -> (varsExp e)++(varsCmd c1)++(varsCmd c2)
			  (Return e)           -> (varsExp e)
			  _                    -> []
...
varsExp e = case (e) of
			  (Var name)           -> [name]
			  (UnaryOp o e1)       -> varsExp e1
			  (BinOp o e1 e2)      -> (varsExp e1)++(varsExp e2)
			  (FunCall name e)     -> vexpList e
			  (ICon _)             -> []
			  (FCon _)             -> []
			  (BCon _)             -> []
...
vexpList :: [Exp] -> [Name]
vexpList [] = []
vexpList (e:rest)  = (varsExp e)++(vexpList rest)